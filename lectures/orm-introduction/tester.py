...
def get_all_posts():
    ...
    #filter by
    posts = Post.query.filter_by(user_id=21).all()
    for post in posts:
        print(post.caption)
...
def get_posts_by_user_ids(user_ids):
    # only want to look at posts from who you follow
...
def get_posts_with_comment_counts():
    # aggregation: how many comments are associated with this specific post?
...
def create_user():
    ...
    user = User(
        ...
        last_name='Goggons',
        username='wgoog',
        email='walter.goggons@gmail.com'
    )
    user.password_plaintext = '12345'
...
#     print(posts)
    ...
    print("----------------------------")
    posts = get_posts_by_username("kati")
    print(posts)
    # for post in posts:
