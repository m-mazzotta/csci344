...
def get_usernames(users):
    ...
    return names
    #can also do return [user.username for user in users]
...
def get_active(users):
    ...
    return active_users
#the new list will only contain active users after the loop finishes
print(get_active(users))

#all in one line:
#return [user for user in users if user.is_active]

# 4. Prints a dictionary representation of the first 3 users
def print_first_three(users):
    for user in users:
        print(user.to_dict())
